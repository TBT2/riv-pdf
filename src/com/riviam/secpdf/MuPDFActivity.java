...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport
{
    ...
    private boolean riviamOpen = false;
    ...
    public void onResume(){
        ...
        boolean rivRunning = false;

        ActivityManager activityManager = (ActivityManager) this.getSystemService( ACTIVITY_SERVICE );
        List<ActivityManager.RunningAppProcessInfo> procInfos = activityManager.getRunningAppProcesses();
        for (ActivityManager.RunningAppProcessInfo procInfo : procInfos) {
            System.out.println("listing " + procInfo.processName);
            if (procInfo.processName.equals("com.riviam.riviamsecure")) {
                rivRunning = true;
            }
        }
        /*

        List<ActivityManager.RecentTaskInfo> spodlus = activityManager.getRecentTasks(100, 0);

        System.out.println(spodlus.size());
        for (ActivityManager.RecentTaskInfo spod : spodlus){
            System.out.println("listing " + spod.description);
            System.out.println("  " + spod.toString());
            try {
                System.out.println("  " + spod.origActivity.toString());
            } catch (Exception e) {
                System.out.println("something is null in Denmark. ");
                e.printStackTrace();
            }
            try {
                System.out.println("  " + spod.origActivity.flattenToString());
            } catch (Exception e) {
                System.out.println("something is null in Denmark. ");
                e.printStackTrace();
            }
            try {
                System.out.println("  " + spod.origActivity.getClassName());
            } catch (Exception e) {
                System.out.println("something is null in Denmark. ");
                e.printStackTrace();
            }
            try {
                System.out.println("  " + spod.baseIntent.toString());
            } catch (Exception e) {
                System.out.println("something is null in Denmark. ");
                e.printStackTrace();
            }
            try {
                System.out.println("  " + spod.describeContents());
            } catch (Exception e) {
                System.out.println("something is null in Denmark. ");
                e.printStackTrace();
            }
        ...
        if(!rivRunning){
            try {
                if (riviamOpen) {
                    System.out.println(" #####  believe we should finish() #####");
//                finish();
                }
            } catch (Exception e){
                System.out.println("core null " + e.getMessage());
            }
        }
    }
	...
	public void onCreate(final Bundle savedInstanceState)
	{
		...
		if (core == null) {
			...
			if (Intent.ACTION_VIEW.equals(intent.getAction())) {
                ...
                if (uri.toString().endsWith("com.riviam.riviamsecure.fileprovider/viewdir/tempFile.pdf")) {
                    riviamOpen = true;
                }
	...
	public void onDestroy()
	{
        ...
        try{
            System.out.println("mupdf unregistering receiver");
